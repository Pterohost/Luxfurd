...
class ConfigManager:
    def __init__(self, g_path: str = "config.yaml", dir_: str = "guilds") -> None:
        ...
        self._global = self._validate_global(self._load_yaml(g_path, {"token": "", "owner_id": 0, "log_level": "INFO"}))
    ...
    def _default_guild() -> dict[str, Any]:
        return {
            "verification": {"captcha_mode": "none", "quarantine_minutes": 0, "min_account_age_days": 0, "require_avatar": False},
            "moderation": {
            ...
            "actions": {
                "spam": "timeout", "repeat": "timeout", "links": "kick", "attachments": "kick",
                "emoji": "timeout", "mentions": "timeout", "require_avatar": "warn",
                "min_account_age": "kick", "bot_join": "ban", "webhook_new": "ban", "captcha_fail": "kick"
            },
            "logging": {"log_channel_id": 0, "alert_channel_id": 0},
            "ignore_roles": [], "ignore_users": []
        }
    ...
    def _validate_guild(config):
        return Schema({
            ...
            "logging": {"log_channel_id": int, "alert_channel_id": int},
            "ignore_roles": [int], "ignore_users": [int]
        }).validate(config)
    ...
    def guild_cfg(self, gid: int) -> MutableMapping[str, Any]:
        if gid not in self._guilds:
            ...
            self._guilds[gid] = self._validate_guild(self._load_yaml(p, self._default_guild()))
            self.save_guild(gid)
        base = self._default_guild()
        for k, v in base.items():
            self._guilds[gid].setdefault(k, v)
